...
// ─────────────────────────────────────────────────────────────
// bruteforce/midgame_search.h — 中盘猜测局面搜索（v6 基线）。
//
// 职责：当前局面没有确定性落子（无安全格、无必标雷）时，选出第一个
// 点击的格子。确定性收尾由搜索以外的既有逻辑处理，这里不碰。
//
// 算法（迭代加深 + 完全展开 + 0.9 硬截断/硬淘汰 + 兜底轮）：
//   树形：招法节点 → 候选（observe 分析单元，结局内嵌）→ 子招法节点。
//   每轮：全部叶子完全展开一层 → 全树自底向上快照 → 0.9 全域斩杀 → 终止检查。
//   终止出口：根候选列表 = 1 / 全终盘 / 预算不足（进入兜底轮）。
//   终止必然性：非终盘叶子每轮必产生子节点（存活候选 ≥ 2 个正概率结局；
//   窗口全死格时钦定生存概率最大的死格为唯一候选），幸存路径深度 ≤
//   未开格数 − 雷数，到点剩余隐藏格全部是雷 → 全终盘。
//
// 状态传递（项目预留的增量机制，structure.h "搜索树节点增量重放"）：
...
//   游走（一次 DFS，路径状态只维护一份）：进入子节点 = 应用其 reveal 与
//   两个 Delta（applyDelta）；退出 = 撤销（applyDelta(reverse=true)，LIFO）
//   ——不再每叶从根重放、不拷贝整盘。可逆性的依据：board 写回 Hidden；
//   Basic::Delta::Change 自带 old（basic.h 注释：供 rollback 逆序恢复）；
//   Structure::Delta 的 removedData 保存被删组件原数据（与 removed 槽位列
//   同序），applyDelta(reverse=true) 按逆序换回。
//   必须缓存、不可重放的产物（否则重放要重复付费）：
//     Position::prob     Exact::analyze 的产物（1 单位，observe 每轮都要）
//     Candidate::observe Exact::observe 的产物（1 单位，死格判定 + 结局分布）
//
// 代价口径：预算 = 真实引擎调用数（时间盒折算），全部引擎调用各计 1 单位：
//   Exact::analyze（局面分析）与 Exact::observe（结局观察）。
//   节点诞生 = 1 分析 + 其窗口逐候选 observe（候选列表形成 + 死格判定 + H0）；
//   展开 = 第一波（每正概率结局子节点 1 分析）。
//   根局面由 get 自带 1 次 Exact::analyze，预算口径自洽。
//
// 常数（阈值放各自层，config.h 只放跨层共享值）：0.9 规则、H0 混合 4:1、
//   次佳兜底 0 均为命名常量，语料校准靠重编译重测（A/B 对照）。
//
...
struct MidgameSearch {
    // ── 搜索树 ──
    // 一次求解的树形数据结构（内部实现，调用方不触碰节点）。
    // 两种节点互持指针；Position 不存完整棋盘，靠 reveal + Delta 沿路径
    // 应用/撤销（游走）维护状态（见文件头"状态传递"）。
    // 生命周期：调用方构造空树传 get，析构自动 DFS 整树回收（防泄漏）；
    // 也可显式 freeAll（析构会再调，幂等）。get 是唯一读写方（外围类
    // 对嵌套类的私有成员有访问权）。
    struct SearchTree {
        // 对外：构造 / 析构 / 显式回收。其余（节点类型与树管理）私有。
        // get 是树状态的唯一读写方（外围类）；C++ 嵌套访问规则下外围类对
        // 嵌套类的私有成员并无特殊访问权（[class.access.nest]），故以
        // friend 显式授予（普通访问规则之外的唯一特例）。
