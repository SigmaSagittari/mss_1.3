...
//
// 稳定性（重要）：基数路径（n > kThreshold）稳定（LSD 计数排序）；小数组
// 分支走 std::sort，不稳定。需要稳定序时调用方须保证键全序（例如附带唯一
// 序号作末字段，见 endgame_bruteforce.h 的 SafeEntry.p），或自行处理等键
// 的输入序。
struct radix_sort {
